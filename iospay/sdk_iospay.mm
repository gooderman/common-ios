...
+ (void) iap_req:(NSString*)pid :(int)numb :(BOOL)force
{
    std::string ppid = [pid UTF8String];
    
    if(sMap.size()==0)
    {
        [sdk iap_notify:-1 :pid :numb :@"" :@"没有商品信息1"];
        return;
    }
    if(sMap.find(ppid)==sMap.end())
    {
        [sdk iap_notify:-2 :pid :numb :@"" :@"没有商品信息2"];
        return;
    }
    if(IOSiAP_Bridge::getInstance()->isBusy())
    {
        ...
        {
            [sdk iap_notify:-3 :pid :numb :@"" :@"正在购买，请稍后"];
            return;
        }
    }
    int flag = IOSiAP_Bridge::getInstance()->requestProducts(ppid,sMap);
    if(flag==1)
    {
        [sdk iap_notify:1 :pid :numb :@"" :@"正在购买，请稍后"];
    }
}
...
+ (void) iap_notify:(int) error :(NSString*)pid :(int)numb :(NSString*)identifer :(NSString*)info
{
