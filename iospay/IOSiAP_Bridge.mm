...
int IOSiAP_Bridge::requestProducts(std::string& productId,std::map<std::string,std::string>& pmap)
{
    if(m_isPaying)
    {
        return 1;
...
void IOSiAP_Bridge::onRequestProductsFinish(void)
{
    ...
    if(m_pmap.find(m_productID) == m_pmap.end())
    {
        ...
        [sdk iap_notify:-2 :pid :1 :@"" :@"没有商品信息3"];
        m_isPaying = false;
        return;
    }
    ...
    {
        NSString* pid = [NSString stringWithUTF8String:m_productID.c_str()];
        [sdk iap_notify:3 :pid :1 :@"" :@"取不到商品信息"];
        m_isPaying = false;
        return;
    }
...
void IOSiAP_Bridge::onRequestProductsError(int code)
{
    ...
    NSString* pid = [NSString stringWithUTF8String:m_productID.c_str()];
    [sdk iap_notify:2 :pid :1 :@"" :@"请求商品信息失败"];
    m_isPaying = false;
}
...
void IOSiAP_Bridge::onPaymentEvent(std::string &identifier, IOSiAPPaymentEvent event, int quantity)
{
    ...
    {
        NSLog(@"付款进行中");
        [sdk iap_notify:1 :pid :1 :pidstr :@"付款进行中"];
        return;
    }
